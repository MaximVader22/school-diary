...
async def send_welcome(message: Message, state: FSMContext):
    ...
    msg = await message.answer("Приветствую! Используйте кнопки для навигации:",
                         reply_markup=create_main_menu(message.from_user.id))
    await main.delete_prev_message(message.from_user.id, msg.message_id)
...
async def handle_edit_remind_time(message: Message, state: FSMContext):
    ...
    if text == "Удалить":
        msg = await message.answer(f"✅ Время напоминания удалено. Вы возвращены в профиль", reply_markup=create_profile_menu())
    ...
    if not is_right_time_format(text):
        msg = await message.answer(f"❌ Укажите время в формате HH:MM")
        return

    set_remind_time(message.from_user.id, text)
    notifier.add_notifier(message.from_user.id, text)
    await state.set_state(Form.idle)
    msg = await message.answer(f"✅ Время напоминания установлено. Вы возвращены в профиль", reply_markup=create_profile_menu())
    await main.delete_prev_message(message.from_user.id, msg.message_id)
...
async def handle_add_subject(message: Message, state: FSMContext):
    if not has_elder_rights(message.from_user.id):
        msg = await message.answer("❌ У вас нет прав старосты")
        return
    try:
        if message.text == "Отмена":
            await state.set_state(Form.idle)
            msg = await message.answer("❌ Вы возвращены в главное меню", reply_markup=create_main_menu(message.from_user.id))
            return
        ...
        if not is_right_time_format(start_time) or not is_right_time_format(end_time):
            msg = await message.answer("❌ Неверный формат ввода времени: HH:MM")
            return

        sch.add_schedule('schedule.json', day, subject, start_time, end_time)
        msg = await message.answer(f"✅ Предмет '{subject}' успешно добавлен на {day}")
        return
    except ValueError:
        msg = await message.answer("❌ Неверный формат ввода. Пожалуйста, используйте формат: день недели, предмет, время начала урока, время конца урока (время в формате ЧЧ:ММ)")
    except AssertionError:
        msg = await message.answer("❌ Неверный формат ввода дня недели")
    await main.delete_prev_message(message.from_user.id, msg.message_id)
...
async def handle_remove_subject(message: Message, state: FSMContext):
    ...
    msg = await message.answer(f"✅ Попытка удалить предмет '{subject}' завершена")
    await main.delete_prev_message(message.from_user.id, msg.message_id)
...
async def handle_add_elder(message: Message, state: FSMContext):
    ...
    if not username_exists(username):
        msg = await message.answer("❌ Не найден пользователь с таким именем")
        return

    set_elder(username, True)
    print(f"Added elder {username}")
    msg = await message.answer(f"✅ {username} назначен старостой")
    await main.delete_prev_message(message.from_user.id, msg.message_id)
...
async def handle_remove_elder(message: Message, state: FSMContext):
    ...
    msg = await message.answer(f"✅ {username} больше не староста")
    await main.delete_prev_message(message.from_user.id, msg.message_id)
...
async def handle_create_announcement(message: Message, state: FSMContext):
    await state.set_state(Form.idle)
    if not has_elder_rights(message.from_user.id):
        msg = await message.answer("❌ У вас нет прав для этого")
        ...
        msg = await message.answer("✅ Объявление создано. Вы возвращены в главное меню",
                             reply_markup=create_main_menu(message.from_user.id))
    await main.delete_prev_message(message.from_user.id, msg.message_id)
...
async def handle_create_event(message: Message, state: FSMContext):
    if not has_elder_rights(message.from_user.id):
        ...
        msg = await message.answer("❌ Неправильный формат сообщения")
    await main.delete_prev_message(message.from_user.id, msg.message_id)
...
async def handle_add_homework(message: Message, state: FSMContext):
    ...
    if len(content) != 3:
        msg = await message.answer("❌ Неверное количество аргументов")
        return
    ...
    if not is_right_homework_date_format(date):
        msg = await message.answer("❌ Неверный формат даты")
        return
    ...
    msg = await message.answer(f"✅ Домашнее задание было добавлено")
    await state.clear()
    await main.delete_prev_message(message.from_user.id, msg.message_id)
...
async def handle_delete_homework(message: Message, state: FSMContext):
    ...
    if message.text == "Подтвердить":
        ...
        if homework_id is not None:
            msg = await message.answer(f"✅ Домашнее задание под номером {homework_id} было успешно удалено")
            await state.update_data({"homework_id": None})
            return

    if not message.text.isnumeric():
        msg = await message.answer("❌ Введите действительное число")
        return

    await state.update_data({"homework_id": int(message.text)})
    msg = await message.answer(f"✅ Если вы уверены, что хотите удалить домашнее задание под номером {message.text}, напишите Подтвердить")
...
async def handle_add_homework_image(message: Message, state: FSMContext):
    ...
    if free_photos_space <= 0:
        msg = await message.answer(f"❌ Вы не можете прикрепить более {max_photos} изображений")
        return
    ...
    msg = await message.answer("✅ Фотография была добавлена")
    await main.delete_prev_message(message.from_user.id, msg.message_id)
...
async def idle(message: types.Message, state: FSMContext):
    await state.set_state(Form.idle)
    msg = await message.answer("Команда не понята. Используйте кнопки для навигации:",
                         reply_markup=create_main_menu(message.from_user.id))
    await main.delete_prev_message(message.from_user.id, msg.message_id)
